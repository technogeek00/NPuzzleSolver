...
function loadAndSolve() {
	var grid = loadPuzzle();
	var solver = new NPuzzleSolver(grid);
	solutioned = solver.solve();
	if(solutioned != null) {
		intervals = setInterval(animationSolution, 50);
		alert("There are " + solutioned.length + " moves to complete this puzzle");
	} else {
		alert("This puzzle has been improperly shuffled, it cannot be solved.");
	}
}

function getOverallSize() {
	var pieces = $('.square'), size = Math.sqrt(pieces.length);
	return size;
}

function loadPuzzle() {
	var size = getOverallSize();
	var grid = [];
...
function animationSolution() {
	if(solutioned.length > 0) {
		var move = solutioned.splice(0, 1)[0];
		var firstID = '#' + move[0].y + '_' + move[0].x;
		var secondID = '#' + move[1].y + '_' + move[1].x;
		var num1 = $(firstID).val();
		var num2 = $(secondID).val();
		$(firstID).val(num2);
		$(secondID).val(num1);
	} else {
